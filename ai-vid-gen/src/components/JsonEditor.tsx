...
export default function JsonEditor({ data, onUpdate, title }: JsonEditorProps) {
	const [editedData, setEditedData] = useState<TimestampTextList>(
		data as TimestampTextList,
	);
	...
	// Synchronize editedData with incoming data prop
	useEffect(() => {
		if (data) {
			setEditedData(data);
		}
	...
	const handleSave = (
		...
		newData: TimestampTextList = editedData,
	) => {
		const invalidTimestamps = newData.items.filter(
			item => !validateTimestamp(item.timestamp),
		);
	...
	const handleAdd = () => {
		...
		if (newRow.timestamp === '' || newRow.text === '') {
			toast({
				...
				description:
					'Please fill in both timestamp and text fields before adding a new row.',
				variant: 'destructive',
			});
			return;
		}
		...
		const newData = { ...editedData };
		const insertIndex = newData.items.findIndex(
			item => item.timestamp > newRow.timestamp,
		);
						...
						<Table>
							<TableHeader>
								<TableRow className="border-none hover:bg-transparent">
									{columns.map(column => (
										<TableHead
											className={`px-4 ${column === 'timestamp' ? 'w-12' : 'w-full'}`}
											key={column}
										>
											{column}
										</TableHead>
